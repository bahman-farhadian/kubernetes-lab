...
Mon + mgr + OSD are co-located on `k8s-work-1/2/3` — 3 mons for quorum, one OSD per node using the dedicated Ceph disk from [02-hardware-inventory.md](../../02-hardware-inventory.md). This is unrelated to the etcd tier above (`k8s-etcd-1/2/3`) — Ceph's own mon/quorum is entirely separate from Kubernetes' etcd.
