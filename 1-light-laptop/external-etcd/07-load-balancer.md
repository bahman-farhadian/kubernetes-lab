...
```
Only 2 backends — this scenario has no `k8s-ctrl-3`. `10.0.1.10` is the VIP clients and `kubeadm` will target — see [03-network-plan.md](../../03-network-plan.md).
...
```
The backend checks will show both control-plane servers as `DOWN` until step 08 actually starts the apiserver on them — that's expected at this point.

## Applies to
Light profile, external etcd — the bastion load-balances across the 2 control-plane nodes only (etcd is a separate tier, not behind this LB).
