...
**Goal:** Bring every VM (bastion, control-plane, etcd, workers, monitor) to a common, Kubernetes-ready OS state.

## Applies to
All nodes: `k8s-bastion`, `k8s-ctrl-1/2`, `k8s-etcd-1/2/3`, `k8s-work-1/2/3`, `k8s-monitor`.
...
**3. Kernel modules + sysctl** — required on control-plane and worker nodes (skip on bastion/monitor). Not required on the etcd-only nodes either — they never run kubelet/containerd (see [08-bootstrap.md](08-bootstrap.md)):
```sh
