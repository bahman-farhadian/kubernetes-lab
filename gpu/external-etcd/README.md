...
**Status:** Not started. Per the [rollout plan](../../README.md#rollout-plan), this profile is built only after **Heavy** (`../../heavy/external-etcd/`) is healthy — GPU is Heavy plus one extra worker (`k8s-work-4`) and one extra step, not a from-scratch build.

## How to build this out

1. Copy every file from `../../heavy/external-etcd/` into this directory (once that one is real, not this stub).
